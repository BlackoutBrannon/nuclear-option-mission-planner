...
// Sight line probe
//
// The origin is a point ON the ground plus a mast, so it represents a site
// rather than an aircraft; the far end is at the altitude on the bar. The
// question it answers is therefore "could something sited here see me where I
// am", which is what the threat rings answer in aggregate and this answers for
// one specific pair of points.
//
// Answers a different question from the threat rings. A ring asks "how far can
// this reach in every direction"; this asks "is the ground clear between these
// two specific points". Both walk the same terrain, but a two-point test can
// compare heights directly rather than angles, because the straight line
// between the ends is known - there is no unknown target to solve for.
//
//   clearance at distance d = height of the straight line there - ground there
//
// Negative clearance is terrain in the way. The minimum over the walk is the
// margin, which is the number worth reading: a line that clears by 20 m is
// clear on this 50 m DEM only in the sense that it has not been disproved.
// ---------------------------------------------------------------------------
let losFrom = null;     // { x, z, alt } or null

function sightLine(a, b) {
    const dist = bearingRange(a, b).range;
    const steps = Math.max(2, Math.ceil(dist / MASK_STEP));

    let minClear = Infinity, minAt = 0;
    let blockedFrom = null, blocked = 0;
    const samples = [];
    ...
    for (let k = 0; k <= steps; k++) {
        ...
        const lineH = a.alt + (b.alt - a.alt) * t;
        const clear = lineH - terrainAt(x, z);

        samples.push(clear);
        if (clear < minClear) { minClear = clear; minAt = dist * t; }
        if (clear < 0) {
            blocked += dist / steps;
            if (blockedFrom === null) blockedFrom = dist * t;
        }
    }

    return { dist: dist, samples: samples, minClear: minClear, minAt: minAt,
             blocked: blocked, blockedFrom: blockedFrom };
}

function drawSightLine(ctx) {
    if (!losFrom || !terrain || !losCursor) return;

    const b = { x: losCursor.x, z: losCursor.z, alt: ownAltM };
    const line = sightLine(losFrom, b);
    const steps = line.samples.length - 1;
    ...
    for (let k = 0; k < steps; k++) {
        ...
        const p0 = toScreen(losFrom.x + (b.x - losFrom.x) * t0,
                            losFrom.z + (b.z - losFrom.z) * t0);
        const p1 = toScreen(losFrom.x + (b.x - losFrom.x) * t1,
                            losFrom.z + (b.z - losFrom.z) * t1);
        const hit = line.samples[k] < 0 || line.samples[k + 1] < 0;
        ...
        // Blocked stretches are heavy and tightly dashed; clear ones are a thin
        // solid line. Weight and pattern, not colour alone.
        ctx.strokeStyle = hit ? '#ffffff' : '#8ecbff';
        ctx.lineWidth   = hit ? 4 : 1.6;
        ctx.setLineDash(hit ? [3, 3] : []);
        ctx.beginPath(); ctx.moveTo(p0.x, p0.y); ctx.lineTo(p1.x, p1.y); ctx.stroke();
    }
    ctx.setLineDash([]);

    // Both ends marked, so the origin is findable once the cursor is far away.
    const o = toScreen(losFrom.x, losFrom.z);
    ctx.beginPath();
...
function sightLineText() {
    if (!losFrom || !terrain || !losCursor) return '';
    const line = sightLine(losFrom, { x: losCursor.x, z: losCursor.z, alt: ownAltM });
    if (line.blocked > 1) {
        return 'LOS BLOCKED from ' + fmtRange(line.blockedFrom) +
               '  (' + fmtRange(line.blocked) + ' of ' + fmtRange(line.dist) + ')';
    }
    return 'LOS CLEAR  ' + fmtRange(line.dist) +
           '  min clearance ' + fmtAlt(line.minClear) +
           ' at ' + fmtRange(line.minAt);
}
